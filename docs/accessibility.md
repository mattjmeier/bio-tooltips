...
Top-level drawers open at the middle reading position (`50dvh`) and expose a separate native **Close** button with a 44px touch target and a resize handle with `role="slider"`, vertical orientation, and an accessible current height. The handle can be dragged up or down to resize the sheet between five resting positions: peek, lower, reading, upper, and expanded; pointer capture is limited to the handle so content scrolling remains normal. Arrow Up/Down resize by keyboard, Home/End select the smallest/largest position, and Page Up/Page Down move between detents. At the expanded position, document scrolling is temporarily locked so scroll input stays within the full-height drawer; leaving that position or closing immediately restores it. Escape and the Close button remain explicit dismissal paths; a deliberate downward gesture from the peek position may also dismiss. Wide-screen popovers retain the existing × close button. Reduced-motion settings remove the handle's snap and drawer transition animations.
